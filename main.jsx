...
export default function App() {
  ...
  // stooq からデータ取得（日本株・米国株両対応・無料・登録不要）
  const fetchStock = useCallback(async (preset) => {
    if(!preset) return;
    setLoading(true); setError(""); setRawData([]); setAiComment(null);
    try {
      // stooqのシンボル変換（日本株は .JP を付ける）
      const stooqSym = preset.market === "JP"
        ? preset.symbol.replace(".T", ".JP")
        : preset.symbol;
      ...
      const stooqUrl = `https://stooq.com/q/d/l/?s=${stooqSym.toLowerCase()}&d1=${fmt(from)}&d2=${fmt(today)}&i=d`;
      ...
      if(!text || text.includes("No data") || text.trim().length < 50) {
        throw new Error("データが見つかりません。銘柄を確認してください。");
      }
      ...
      if(parsed.length === 0) throw new Error("データが空です。銘柄を確認してください。");
      setRawData(parsed);
    } catch(e) {
